...
def do_hico_evaluation(cfg, epoch, dataset, predictions, output_folder, logger, iou_types):

    preds, gts = [], []
    for image_id, boxes_list in enumerate(predictions['graph_obj_box_lists']):
        img_info = dataset.get_img_info(image_id)
        image_width = img_info["width"]
        image_height = img_info["height"]
        boxes_list = boxes_list.resize((image_width, image_height))
        gt_boxes = dataset.get_groundtruth(image_id, evaluation=True)

        preds.append({'boxes': boxes_list.bbox, 
                      'labels': boxes_list.get_field('pred_labels'), 
                      'verb_scores': boxes_list.get_field('rel_cls_scores'),
                      'sub_ids': boxes_list.get_field('rel_pair_idxs')[:, 0],
                      'obj_ids': boxes_list.get_field('rel_pair_idxs')[:, 1],
                      })
        gts.append({'boxes': gt_boxes.bbox,
                    'labels': gt_boxes.get_field('labels'),
                    'hois': gt_boxes.get_field('relation_tuple')})


    with open('/public/sist/home/FWS20210094/projects/scene_graph/SGGTR/sggtr/data/datasets/evaluation/hico/rare_triplets.json', 'r') as f:
        triplets = json.load(f)
    rare_triplets = [tuple(t) for t in triplets['rare_triplets']]
    non_rare_triplets = [tuple(t) for t in triplets['non_rare_triplets']]
    correct_mat = np.load('/public/sist/home/FWS20210094/projects/scene_graph/SGGTR/sggtr/data/datasets/evaluation/hico/corre_hico.npy')

    evaluator = HICOEvaluator(preds, gts, 0, rare_triplets, non_rare_triplets, correct_mat)
    ...
    def __init__(self, preds, gts, subject_category_id, rare_triplets, non_rare_triplets, correct_mat):
        ...
        for img_preds in preds:
            ...
            if len(subject_ids) > 0:
                ...
                drop = verb_labels > 117
                verb_labels[drop] = 0
                masks = correct_mat[verb_labels - 1, object_labels - 1]
                masks[drop] = 0
                hoi_scores *= masks
                hoi_scores[subject_labels != 1] = 0

                # keep = hoi_scores > 0
                # subject_ids = subject_ids[keep]
                # object_ids = object_ids[keep]
                # verb_labels = verb_labels[keep]
                # hoi_scores = hoi_scores[keep]
    ...
    def compute_fptp(self, pred_hois, gt_hois, match_pairs, pred_bboxes, bbox_overlaps):
        ...
        if len(pred_hois) != 0:
            for pred_hoi in pred_hois:
                ...
                if triplet not in self.gt_triplets: # or (is_match and vis_tag[gt_hois.index(max_gt_hoi)] == 1):
                    continue
