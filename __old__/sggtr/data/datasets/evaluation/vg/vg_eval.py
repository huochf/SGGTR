...
def do_vg_evaluation(cfg, epoch, dataset, predictions, output_folder, logger, iou_types):
    # get zeroshot triplet
    zeroshot_triplet = torch.load("/public/sist/home/FWS20210094/projects/scene_graph/SGGTR/sggtr/data/datasets/evaluation/vg/zeroshot_triplet.pytorch", map_location=torch.device("cpu")).long().numpy()
    ...
    save_output(epoch, output_folder, groundtruths, predictions, dataset)
...
def save_output(epoch, output_folder, groundtruths, predictions, dataset):
    if output_folder:
