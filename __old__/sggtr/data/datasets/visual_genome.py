...
class VGDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None, filter_empty_rels=True,
                 num_im=-1, num_val_im=5000, filter_duplicate_rels=True, filter_non_overlap=True, flip_aug=False,
        ...
        self.ind_to_classes, self.ind_to_predicates, self.ind_to_attributes = load_info(dict_file) # contigous 151, 51 containing __background__
        ...
        if self.custom_eval:
            ...
            self.split_mask, self.gt_boxes, self.gt_classes, self.gt_attributes, self.relationships = load_graphs(
                ...
                filter_empty_rels=filter_empty_rels, filter_non_overlap=self.filter_non_overlap,
            )
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        if flip_img:
            ...
            box[:, 0] = new_xmin
            box[:, 2] = new_xmax
        target = BoxList(box, (w, h), 'xyxy', False) # xyxy
        ...
        target.add_field("relation_tuple", torch.LongTensor(relation), is_triplet=True) # for evaluation
        return target
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True):
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file, dict_file=dict_file,
        image_file=image_file, num_val_im=5000, filter_duplicate_rels=False)
...
def bbox_overlaps(boxes1, boxes2, to_move=1):
    ...
    rb = np.maximum(boxes1.reshape([num_box1, 1, -1])[:, :, 2:], boxes2.reshape([1, num_box2, -1])[:, :, 2:]) # [N, M, 2]
...
def load_image_filenames(img_dir, image_file):
    """
    Loads the image filenames from visual genome from JSON file that contains them.
    ...
    for i, img in enumerate(im_data):
        ...
        if os.path.exists(filename):
            ...
            filename = os.path.join(img_dir + '_2', basename)
            if os.path.exists(filename):
                fns.append(filename)
                img_info.append(img)
            else:
                raise RuntimeError("image file {} doesn't exist".format(filename))
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    ...
    Parameters:
        ...
        filter_non_overlap: If training, filter images that don't overlap.
    Return:
