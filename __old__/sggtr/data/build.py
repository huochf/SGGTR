...
from sggtr.utils.miscellaneous import save_labels

# by Jiaxin
def get_dataset_statistics(cfg):
    """
    get dataset statistics (e.g., frequence bias) from training data
...
def build_dataset(cfg, dataset_list, transforms, dataset_catalog, is_train=True):
    ...
    if not isinstance(dataset_list, (list, tuple)):
        raise RuntimeError(
            "dataset_list should be a list of string, got {}".format(dataset_list)
        )
    datasets = []
    for dataset_name in dataset_list:
        ...
        factory = getattr(D, data["factory"])
        args = data["args"]
        # for COCODataset, we want to remove images without annotaions during training
...
def make_data_loader(cfg, mode='train', is_distributed=False, start_iter=0):
    ...
    if is_train:
        ...
        ), "SOLVER.IMS_PER_BATCH ({}) must be divible by the number of GPUs ({}) used.".format(
    ...
    if is_train:
        # save catelogy_id to label name mapping
    ...
    for dataset in datasets:
        ...
        if cfg.TEST.CUSTOM_EVAL:
            ...
            print('====> ' + str(os.path.join(cfg.DETECTED_SGG_DIR, 'custom_data_info.json')))
